...
def register_tag_tools(mcp, session_manager: SessionManager, runner: AsepriteRunner):
    ...
    def get_tags(session_id: str) -> dict:
        ...
        result = runner.run_script("get_tags.lua", {"file": str(ase_path)})
        tags, error = parse_json_output(result, "Failed to get tags")
        if error:
            return error
